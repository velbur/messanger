...
Промпты к кадрам: **Grok LLM** (`XAI_MODEL`) читает **всю переписку** и формирует `imagePrompt`. Генерация вложений в чат: **4:3** (не Full HD 9:16), **Kling** / **Grok Imagine** (`grok-imagine-image` по умолчанию — дешевле quality).
