...
# Требует клон репозитория messanger, Node 20+, ffmpeg.
#
# На Mac в docs/.env:
#   LOCAL_GPU_RENDER_URL=http://<gpu-host>:3333
#   # или LOCAL_GPU_RENDER_AUTO=1 + LOCAL_GPU_VIDEO_URL=http://<gpu-host>:8008
set -euo pipefail
...
echo "Откройте порт ${PORT} в firewall провайдера (наряду с 8008)."
