...
export const DEFAULT_STYLE_PROMPT = `Единый стиль проекта: детализированная рисованная иллюстрация (digital painting), чёткие уверенные контуры, богатая проработка деталей и фактур, мягкая живописная заливка, выразительный мягкий свет и тени, насыщенная, но гармоничная палитра, тёплая уютная атмосфера. Не фотореализм, не 3D-рендер, не аниме, не вектор. Без текста и надписей на картинке.`;

export const DEFAULT_STORY_STYLE_PROMPT = `Единый стиль проекта: детализированная рисованная иллюстрация (digital painting), чёткие уверенные контуры, богатая проработка деталей и фактур, мягкая живописная заливка, выразительный кинематографичный свет и атмосфера, насыщенная, но гармоничная палитра. Кадр как иллюстрация художника-сториборда: глубина и объём сцены, продуманная композиция. Не фотореализм, не 3D-рендер, не аниме, не вектор. Без текста и надписей на картинке.`;
...
export const buildFullDialogueTranscriptForLlm = (
  ...
  const formatLine = (msg, index, {isFuture = false} = {}) => {
    ...
    if (index === messageIndex) {
      mark = " ← ЦЕЛЕВОЕ СООБЩЕНИЕ (генерируем иллюстрацию к нему)";
    } else if (isFuture) {
...
export const buildHeuristicScenePrompt = ({
  ...
  return assemblePrompt([
    {
      key: "task",
      text: `По переписке ниже сгенерируй одну иллюстрацию ТОЛЬКО для сообщения ${messageIndex + 1} (${who}, пометка ←СГЕНЕРИРУЙ ЭТО ФОТО). Остальное — контекст, не рисовать.`,
    },
    {
      key: "dialogue",
