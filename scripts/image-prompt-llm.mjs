...
const buildStorySystemPrompt = ({hasReferences = false, stylePrompt = ""} = {}) => {
  ...
  return [
    ...
    style
      ? `Общий стиль всех кадров (обязательно): ${style}`
      : "Стиль: детализированная painterly-иллюстрация, сториборд, не мультфильм и не фотореализм.",
    "Опиши establishing shot / атмосферу момента: конкретные детали интерьера, света, поз, одежды, выражений.",
    "Запрещены формулировки «фото», «фотореализм», «снято на камеру», а также «мультик», «cartoon», «упрощённо».",
    hasReferences
...
export const buildStoryImageGenerationPrompt = ({imagePrompt, stylePrompt}) => {
  ...
  return [
    scene,
    style ? `Стиль: ${style}` : "",
    `Детализированная painterly-иллюстрация для взрослой аудитории (digital painting, concept art), не мультфильм и не cartoon. Вертикальный формат ${STORY_IMAGE_ASPECT_RATIO} на весь экран, без UI чата и без текста. Запрещены фото, фотореализм, упрощённая мультяшность.`,
  ]
    .filter(Boolean)
    .join(" ");
};
